...
export async function renderCard(canvas: HTMLCanvasElement, o: CardOptions) {
  ...
  if (photo) {
    ...
    if (!baked) {
      ...
      const lw = W * 0.8, lh = lw * logo.height / logo.width;
    ...
    g = ctx.createLinearGradient(0, H * 0.58, 0, H); g.addColorStop(0, 'rgba(0,0,0,0)'); g.addColorStop(1, 'rgba(0,0,0,.85)');
    ...
    const lw = W * 0.74, lh = lw * logo.height / logo.width;
    ctx.drawImage(tint(logo, ink), (W - lw) / 2, 170, lw, lh);
    ctx.fillStyle = ink; ctx.textAlign = 'center'; ctx.textBaseline = 'alphabetic'; spaced(ctx, 0);
    fit(ctx, lm.name, s => `italic ${s}px ${serif}`, 128, W * 0.7);
    ctx.fillText(lm.name, W / 2, 1030);
    ctx.globalAlpha = 0.35; ctx.fillRect(130, 1105, W - 260, 3); ctx.globalAlpha = 1;
  }
  ...
  if (avatar) { ctx.clip(); cover(ctx, avatar, ax - R, fy - R, 2 * R, 2 * R); }
    ...
    ctx.fillText(initials(o.name), ax, fy + 6);
  }
  ctx.restore();
  ctx.beginPath(); ctx.arc(ax, fy, R + 9, 0, Math.PI * 2); ctx.strokeStyle = photo ? col : ink; ctx.lineWidth = 7; ctx.stroke();

  const tx = ax + R + 50;
  ctx.fillStyle = fg; ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic'; spaced(ctx, 0);
  fit(ctx, o.name, s => `${s}px ${serif}`, 96, 1150);
  ctx.fillText(o.name, tx, fy + 12);
  ctx.font = `500 29px ${mono}`; spaced(ctx, 4); ctx.globalAlpha = 0.88;
  ctx.fillText(((o.handle ? '@' + o.handle + '  ·  ' : '') + 'CHOSE ' + lm.name).toUpperCase(), tx + 3, fy + 64);

  ctx.textAlign = 'right';
  ctx.fillText('SOLANA BREAKPOINT 2026', W - 130, fy + 12);
  ctx.fillText('OLYMPIA LONDON · 15–17 NOV', W - 130, fy + 64);
  ctx.restore();
}
